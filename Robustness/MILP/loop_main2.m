 ...
 function [lower_bound, upper_bound] = loop_main2(n_neuron, lower_bound, upper_bound, depth_ind, w_b_net)
...
% %         x_b = binvar(1,lay_neuron_size(1));
        ...
        out{depth_ind-1} = (((x{depth_ind-1} * w{depth_ind-1}(:,n_neuron) + b{depth_ind-1}(n_neuron)) - mean{depth_ind-1}(n_neuron))./(sqrt(var{depth_ind-1}(n_neuron)+1e-5))).*gamma{depth_ind-1}(n_neuron)+beta{depth_ind-1}(n_neuron);
        ...
        constraints = [constraints, -1 <= x{1} , x{1} <= 1];
%         constraints = [constraints, 0 <= x{1} , x{1} <= repmat(x_b,[input_size 1]), sum(x_b)<=2];

        % Define the objective
        objective =   out{depth_ind-1};
        options = sdpsettings('solver','gurobi', 'gurobi.IntFeasTol', 1e-9, 'gurobi.Threads', 0, 'verbose', 0, 'savesolveroutput',1,'gurobi.TimeLimit', 150);
        sol = optimize(constraints, objective, options);
        lower_bound = double(sol.solveroutput.result.objbound)-1e-8;

        % Define the objective
        objective =   -out{depth_ind-1};
        options = sdpsettings('solver','gurobi', 'gurobi.IntFeasTol', 1e-9, 'gurobi.Threads', 0, 'verbose', 0, 'savesolveroutput',1,'gurobi.TimeLimit', 150);
        sol = optimize(constraints, objective, options);
        upper_bound = -double(sol.solveroutput.result.objbound)+1e-8;
 end
