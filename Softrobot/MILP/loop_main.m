 ...
 function [lower_bound, upper_bound] = loop_main(n_neuron, lower_bound_all, upper_bound_all, depth_ind, w_b_net)
...
%         x_b = binvar(1,lay_neuron_size(1));
        ...
        for lay =1:depth_ind-2
            constraints = [constraints,
                x{lay+1} >= out{lay},...
                x{lay+1} >= 0, ...
                x{lay+1} <= repmat(upper_bound_all{lay+1},[input_size 1]).*z{lay}, ...
                x{lay+1} <= out{lay}-repmat(lower_bound_all{lay+1},[input_size 1]).*(1-z{lay}), ...
                ];
        end

        % Define the selection constraints.
        constraints = [constraints, -0.2<= x{1} , x{1} <= 0.2];
...
%         constraints = [constraints, 0 <= x{1} , x{1} <= repmat(x_b,[input_size 1]), sum(x_b)<=2];
        ...
        options = sdpsettings('solver','gurobi', 'gurobi.IntFeasTol', 1e-9, 'gurobi.Threads', 0, 'savesolveroutput',1,'gurobi.TimeLimit', 500);
